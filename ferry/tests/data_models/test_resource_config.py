...
def test_invalid_append_with_strategy():
    with pytest.raises(
        ValidationError,
        match="No strategy or config is accepted when write_disposition type is 'append'",
    ):
...
def test_merge_scd2_strategy():
    ...
    config = ResourceConfig(
        ...
        write_disposition_config=WriteDispositionConfig(
            type=WriteDispositionType.MERGE.value,
...
def test_invalid_write_disposition_type():
    ...
    assert (
        "value is not a valid enumeration member" in error_message
        or "not a valid WriteDispositionType"
    )
