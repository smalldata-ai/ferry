...
def test_valid_delete_insert_merge_config(valid_delete_insert_merge_config):
    ...
    assert (
        config.delete_insert_config.merge_key
        == valid_delete_insert_merge_config["delete_insert_config"]["merge_key"]
    )
    assert (
        config.delete_insert_config.primary_key
        == valid_delete_insert_merge_config["delete_insert_config"]["primary_key"]
    )
    assert (
        config.delete_insert_config.hard_delete_column
        == valid_delete_insert_merge_config["delete_insert_config"]["hard_delete_column"]
    )
    assert (
        config.delete_insert_config.dedup_sort_column["ep_id"].value
        == valid_delete_insert_merge_config["delete_insert_config"]["dedup_sort_column"]["ep_id"]
    )
...
def test_successfully_ingest_with_upsert(valid_upsert_config):
    ...
    assert (
        config.upsert_config.hard_delete_column
        == valid_upsert_config["upsert_config"]["hard_delete_column"]
    )


def test_upsert_config_mandatory_when_none(valid_upsert_config):
