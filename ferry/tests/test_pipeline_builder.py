...
def test_build_pipeline(mock_dlt_pipeline, ingest_data):
    ...
    mock_dlt_pipeline.assert_called_once_with(
        pipeline_name=ingest_data.identity,
        dataset_name=ANY,  
        destination=ANY,
        progress=ANY
    )
...
def test_run_pipeline(mock_dlt_pipeline, ingest_data):
    ...
    builder.source.dlt_source_system = MagicMock(return_value="mock_resource")  # Return a single string
    ...
    builder.source.dlt_source_system.assert_called_once_with(
        uri=ingest_data.source_uri,
        resources=ingest_data.resources,
        identity=ingest_data.identity
    )

    
    mock_data = builder.source.dlt_source_system()
    flattened_data = [item for sublist in mock_data for item in (sublist if isinstance(sublist, list) else [sublist])]
