...
class LoadDataRequest(BaseModel):
    source_uri: str = Field(..., description="URI of the source database or S3 bucket")
    destination_uri: str = Field(..., description="URI of the destination database")
    source_table_name: str = Field(..., description="Name of the source table or file path in S3")
    ...
    def validate_source_uri(cls, v: str) -> str:
        ...
        return cls._validate_uri(v, ["postgresql", "clickhouse", "duckdb", "s3"], "Source Database")
    ...
    def _validate_uri(cls, v: str, allowed_schemes: list, db_type: str) -> str:
        """Validates that the URI follows the expected format for PostgreSQL, ClickHouse, DuckDB, or S3."""
        parsed = urlparse(v)
        ...
        if scheme == "s3":
            ...
            required_params = ["access_key_id", "access_key_secret", "region", "bucket_name"]
            missing_params = [p for p in required_params if p not in query_params]

            if missing_params:
                raise ValueError(f"S3 URI is missing required parameters: {', '.join(missing_params)}")

            return v

        # PostgreSQL and ClickHouse validation
